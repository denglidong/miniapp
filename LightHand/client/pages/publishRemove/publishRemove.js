...
Page({
  data: {
    ...
    // 产品规格结束
    alibaba_category_id_path: '',//平台分类id字符串
    // publish页面跳转传递过来的产品属性值
    no_sku: [],
    // publish页面跳转传递过来的产品规格值
    have_sku: [],
    // 基本信息开始
    group: [],//产品分组数据
    ...
    deleteIndex_sale: '',
    // array_ys: [
    //   { name: '直接下单品', id: 0, english: 'wholesale' },
    //   { name: '非直接下单品', id: 1, english: 'sourcing' }
    // ],
    // index_ys: 0,
    // 底部弹起多选属性点击下标值
    indexDouble: 0,
  ...
  onTopBtnTap(e) {
    ...
    console.log(this.data.showTop1);

    // 产品属性多选变色开始
    const key = this.data.no_sku[this.data.idx].en_name;
  ...
  onTopGuige(e) {
    this.setData({
      showTop2: true,
      indexGuige: e.target.dataset.indexGuige
    });
  },
  onLoad(e) {
    my.setNavigationBar({
    ...
    if (e.id != undefined) {
      const id_path = e.id_path
      const produceId = e.produceId
      const id = parseInt(e.id)
      const product_type = e.product_type
      const value = JSON.parse(e.value)
      const valueStr = value.join('/')
      const modelName = e.modelName
      const have_sku = app.globalData.haveSkuRevampHello;
      const no_sku = app.globalData.noSkuRevampHello;
      const group = app.globalData.group
      const headstream = this.data.headstream;    //产品属性发源地subset值添加
      console.log(no_sku, '123');
      console.log(have_sku, '222');
      for (const key of no_sku) {
        if (key.en_name == 'Place of Origin') {
          key.subset = headstream
        }
      }
      const productStandard = this.data.productStandard;//产品规格subset值添加
      for (const keyValue of have_sku) {
        if (keyValue.input_type != 'input') {
          keyValue.subset = productStandard
        }
      }
      this.setData({
        modelStyle: valueStr,
        ...
        category_id: id,
        produceId: produceId,
        alibaba_category_id_path: id_path,
        group: group
      })
      // 产品属性赋值开始   
      let attr = { ...this.data.projectAttr };
      for (let index = 0; index < this.data.no_sku.length; index++) {
        if (this.data.no_sku[index].input_type == 'multi_select') {
          let key = this.data.no_sku[index].en_name
          attr[key] = []
        } else {
          let key = this.data.no_sku[index].en_name
          let value = {
            attribute_id: this.data.no_sku[index].attribute_id,
            attribute_name: this.data.no_sku[index].en_name,
            value_id: '',
            value_name: ''
          }
          attr[key] = value
      ...
      for (let index = 0; index < this.data.have_sku.length; index++) {
        if (this.data.have_sku[index].input_type == 'input') {
          let key = this.data.have_sku[index].en_name
          let value = {
            attribute_id: this.data.have_sku[index].attribute_id,
            attribute_name: this.data.have_sku[index].en_name,
            local_option: ''
          }
          standard[key] = value
        } else {
      ...
      const have_sku = app.globalData.tempHaveSku;
      const no_sku = app.globalData.tempNoSku;
      const group = app.globalData.group
      console.log(no_sku);
      const headstream = this.data.headstream;
      for (const iterator of no_sku) {
      ...
      console.log(this.data.group, '赋值');
      // 产品规格赋值
      let standard = { ...this.data.standardCollection }
      for (let index = 0; index < this.data.have_sku.length; index++) {
        if (this.data.have_sku[index].input_type == 'input') {
      ...
      console.log(this.data.projectAttr, '产品属性页面值');
      console.log(this.data.payment_ename, '付款方式');
      //产品属性赋值结束
  ...
  selectDanxuan(e) {
    ...
    // 产品属性多选变色结束
    if (this.data.no_sku[this.data.idx].input_type === "single_select") {
      ...
      if (attr[key].length != 0) {
        if (attr[key].map(item => item.value_id).includes(value_id)) {
          ...
          attr[key].push({
            attribute_id: this.data.no_sku[this.data.idx].attribute_id,
            attribute_name: this.data.no_sku[this.data.idx].en_name,
            value_id: value_id,
    ...
    console.log(this.data.projectAttr, '所有的值');
  },
  //获取发布模板详情数据
  async getApprovalProductList() {
    try {
      const result = await cloud.application.httpRequest({
        ...
        'method': 'POST',
        'body': {
          "api": "/api/publish.tpl/" + this.data.produceId,
          "method": "GET",
          "token": app.globalData.token,
          "data": {
            "id": this.data.produceId
          }
        },
        'exts': { "timeout": 10000 }
      });
      //运费模板默认值
      for (let index = 0; index < this.data.templateFreight.length; index++) {
        if (this.data.templateFreight[index].selectedId === result.data.data.wholesale.shipping_line_template_id) {
          this.setData({
            templateId: this.data.templateFreight[index].selectedId,
            templateName: this.data.templateFreight[index].text
          })
        }
      }
      // 产品分组默认值赋值
      for (const iterator of this.data.group) {
        if (iterator.group_id == result.data.data.group_id) {
          this.setData({
            groupId: result.data.data.group_id,
            groupName: iterator.group_name
          })
        }
      }
      // let payment_methids = []
      // if (result.data.data.sourcing.payment_methods.length != 0) {
      //   payment_methids = result.data.data.sourcing.payment_methods.length
      // }
      // 尺寸截取值
      const package_size = result.data.data.wholesale.package_size.split('X');
      this.setData({
        // 平台分类id字符串
        alibaba_category_id_path: result.data.alibaba_category_id_path,
        package_size: package_size,
        modelName: result.data.name,
        modelStyle: result.data.alibaba_category_zh_name_path,
        product_type: result.data.data.product_type,
        alibaba_category_id: result.data.alibaba_category_id,
        //定制信息内容
        custom: result.data.data.custom,
        sourcing: result.data.data.sourcing,
        wholesale: result.data.data.wholesale,
        noSku: result.data.data.attributes.no_sku,
        // havaSku: result.data.data.attributes.have_sku,
        sale_ename: result.data.data.wholesale.sale_type,
        calculate_ename: result.data.data.wholesale.unit_type,
        discount: result.data.data.discount,
        deliver_periods: result.data.data.wholesale.deliver_periods,

        currency_ename: result.data.data.sourcing.fob_currency,
        indirect_calulate_ename: result.data.data.sourcing.min_order_unit_type,
        payment_ename: result.data.data.sourcing.payment_methods,
        // 直接下单物流运费
        calculate_f_ename: result.data.data.sourcing.fob_unit_type,
        measure_of_availability_ename: result.data.data.sourcing.supply_unit_type,
        Availability_cycle_ename: result.data.data.sourcing.supply_period_type,
        deliver_i_periods: result.data.data.sourcing.deliver_periods,

        // 获取模板的平台分类id
        category_id: result.data.data.category_id,
      });
      console.log(result.data.data.wholesale.deliver_periods, '111111');
      // 产品类型默认值设置
      if (result.data.data.product_type == 'sourcing') {
        this.setData({
          productStyleEnglishName: 'sourcing',
          productStyleChineseName: '非直接下单品'
        })
      } else {
        this.setData({
          productStyleEnglishName: 'wholesale',
      ...
      console.log(result.data.data.product_type);
      const noSku = result.data.data.attributes.no_sku;
      let attr = { ...this.data.projectAttr };
      for (const key in this.data.projectAttr) {
        for (let index = 0; index < noSku.length; index++) {
          if (noSku[index].attribute_name == key) {
            if (Array.isArray(this.data.projectAttr[key])) {
              if (noSku[index].value_id != '') {
                let keyName = noSku[index].attribute_name
                let value = {
                  attribute_name: noSku[index].attribute_name,
                  attribute_id: noSku[index].attribute_id,
                  value_name: noSku[index].value_name,
                  value_id: noSku[index].value_id
                }
                attr[keyName].push(value)
              }
            } else {
              let keyName = noSku[index].attribute_name
              let value = {
      ...
      // 产品规格默认值
      const haveSku = result.data.data.attributes.have_sku;
      let standard = { ...this.data.standardCollection };
      for (const key in this.data.standardCollection) {
        for (let index = 0; index < haveSku.length; index++) {
          if (haveSku[index].attribute_name == key) {
            let key = haveSku[index].attribute_name
            let local_option = haveSku[index].local_option
            standard[key].local_option = local_option
          } else {
          }
        }
      }
  ...
  toSelectType(e) {
    my.navigateTo({
      url: '../revampHello/revampHello?modelStyle=' + this.data.modelStyle + "&modelName=" + this.data.modelName + '&alibaba_category_id_path='
        + this.data.alibaba_category_id_path + "&product_type=" + this.data.product_type + "&produceId=" + this.data.produceId,
      success: (res) => {
        console.log('跳转成功');
      }
    });
        ...
        'body': {
          ...
          "data": {
            ...
            'sourcing': {
              ...
              'delivery_port': this.data.sourcing.delivery_port,
              'delivery_time': this.data.sourcing.delivery_time,
              'fob_currency': this.data.currency_ename,
              'fob_unit_type': this.data.calculate_f_ename,
              'min_order_quantity': this.data.sourcing.min_order_quantity,
              'min_order_unit_type': this.data.indirect_calulate_ename,
              'packaging_desc': this.data.sourcing.packaging_desc,
              'payment_methods': this.data.payment_ename,
              'supply_period_type': this.data.Availability_cycle_ename,
              'supply_quantity': this.data.sourcing.supply_quantity,
              'supply_unit_type': this.data.measure_of_availability_ename
            },
            'wholesale': {
              'batch_number': this.data.wholesale.batch_number,
  ...
  supply_quantity(event) {
    ...
    this.setData({
      ['sourcing.supply_quantity']: supply_quantity
    })
  },
  // 非直接下单包装信息失去焦点
  packaging_desc(event) {
    ...
    this.setData({
      ['sourcing.packaging_desc']: packaging_desc
    })
  },
  //  非直接下单发货期限失去焦点
  delivery_time(event) {
    ...
    this.setData({
      ['sourcing.delivery_time']: delivery_time
    })
  },
  // 非直接下单最小起订量失去焦点
  min_order_quantity(event) {
    ...
    this.setData({
      ['sourcing.min_order_quantity']: min_order_quantity
    })
  },
  // 非直接下单发货港口失去焦点
  delivery_port(event) {
    ...
    this.setData({
      ['sourcing.delivery_port']: delivery_port
    })
  },
  ...
  // change() {
  //   this.getApprovalProductYunFei()
  // },


  async getApprovalProductYunFei() {
    try {
      const result = await cloud.application.httpRequest({
        'path': 'index.php',
        'method': 'POST',
  ...
  productStandard(event) {
    ...
    const index = event.target.dataset.index
    // const value_name = event.target.dataset.value_name
