...
Page({
  ...
  onLoad(options) {
    ...
    const product_type = options.product_type
    const modelName = options.modelName
    let value = []
    // 新建页面切换带过来的平台分类id字符串
    const id_path = options.alibaba_category_id_path.split('/')
    // 新建页面切换带过来的平台分类value字符串
    if (options.modelStyle != "") {
      value = options.modelStyle.split('/')
    } else {
      value = []
    }
    // 数组长度
  ...
  toPublishRemove() {
    console.log(this.data.id_path, '123');

    let value = JSON.stringify(this.data.value)
    my.navigateTo({
      url: '../publishRemove/publishRemove/?id=' + this.data.id + "&id_path=" + this.data.id_path + "&value=" + value + "&modelName=" + this.data.modelName + "&product_type=" + this.data.product_type + "&produceId=" + this.data.produceId,
    })
  ...
  async GetAttr() {
    ...
    try {
      const result = await cloud.application.httpRequest({
        ...
        'method': 'POST',
        'body': {
          "api": "/api/common/aliattribute/" + this.data.id,
          "method": "GET",
          "token": app.globalData.token,
          "data": {
            'id': -1
      ...
      const noSkuRevampHello = result.data.no_sku
      const haveSkuRevampHello = result.data.have_sku
      app.globalData.noSkuRevampHello = noSkuRevampHello
      app.globalData.haveSkuRevampHello = haveSkuRevampHello
